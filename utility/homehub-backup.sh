...
function diskHandler()
	{	
		if [[ $1 == "mount" ]]; then
			mkdir -m u=wrx,g=rx,o=rx -p backups # make ifnotexists dir where disk will be mounted
			mount -L $disklabel backups #-o umask=0022 # mount disk with permission u=rwx,g=rx,o=rx
			# chek if the disk is correctly mounted
			if [[ $? -ne 0 ]]; then
				errorHandler "out" "Problem to mount the backup volume"
			fi

			mkdir -m 755 -p backups/tmp # make ifnotexists the temp dir into the drive
			echo -e "${GREEN}---------- DISK MOUNTED ----------${NC}"

		elif [[ $1 == "unmount" ]]; then
			umount backups # unmount disk
...
function maintenance()
	{
		...
		if [[ $1 == true ]]; then
			sed -i "s/'maintenance' => false/'maintenance' => true/g" boxhub/config/config.php
			if [[ $? -ne 0 ]]; then 
		...
		# remove cloud from maintenance mode
			...
			sed -i "s/'maintenance' => true/'maintenance' => false/g" boxhub/config/config.php
...
function sql_backup()
	{
		...
		docker cp homehub_sql_1:/${sqlbk}_$d.sql backups/tmp/ # copy sql file into host
		if [[ $? -ne 0 ]]; then
...
function box_backup()
	{
		...
		echo
		rsync -Aax --info=progress2 boxhub/ backups/tmp/${databk}_$d/ # copy all nextcloud data
		if [[ $? -ne 0 ]]; then
...
function ftp_backup()
	{
		...
		rsync -Aax --info=progress2 ftp/ backups/tmp/${ftpbk}_$d/ #copy all ftp data
		if [[ $? -ne 0 ]]; then
...
function compress()
	{
		...
		echo
		tar -C backups/tmp/ -cvf backups/homehub-backup_$d.tar ${databk}_$d ${sqlbk}_$d.sql ${ftpbk}_$d # compress data and sql in an archive
		if [[ $? -ne 0 ]]; then
			errorHandler "out" "Problem creating the archive"
		fi
		echo -e "${GREEN}---------- COMPRESSING ----------${NC}"
		gzip -4 backups/homehub-backup_$d.tar
		if [[ $? -ne 0 ]]; then
...
function clean()
	{
		...
		rm -rvf backups/tmp/* # delete data and sql in tmp dir
